...
def parse_int(value: str, default: Optional[int] = None) -> int:
    ...
    try:
        ...
        if default is not None:
            logger.warning(f"Failed to parse '{value}' to int, using default: {default}")
            return default
        else:
            raise
...
def parse_float(value: str, default: Optional[float] = None) -> float:
    ...
    try:
        ...
        if default is not None:
            logger.warning(f"Failed to parse '{value}' to float, using default: {default}")
            return default
        else:
            raise
...
def validate_config(config):
    ...
    if "api_key" in ocr_config and ocr_config["api_key"] is not None and not isinstance(ocr_config["api_key"], str):
        logger.error("OCR api_key must be a string or null")
        return False
    
    # Validate evaluation section
    eval_config = config.get("evaluation", {})
    if "num_evaluations" in eval_config and not isinstance(eval_config["num_evaluations"], int):
    ...
    if "temperature" in eval_config and not isinstance(eval_config["temperature"], (int, float)):
    ...
    if "api_key" in eval_config and eval_config["api_key"] is not None and not isinstance(eval_config["api_key"], str):
    ...
    if "multi_provider_evaluation" in eval_config:
        ...
        if eval_config["multi_provider_evaluation"] and "providers_config" in eval_config:
            ...
            for run_key, run_config in eval_config["providers_config"].items():
                if not isinstance(run_config, dict):
                    logger.error(f"Evaluation providers_config.{run_key} must be a dictionary")
                    return False
                
                # Validate required fields in run configuration
                if "provider" in run_config and not isinstance(run_config["provider"], str):
                    logger.error(f"Evaluation providers_config.{run_key}.provider must be a string")
                    return False
                
                if "model" in run_config and not isinstance(run_config["model"], str):
                    logger.error(f"Evaluation providers_config.{run_key}.model must be a string")
                    return False
    
    # Validate interpreter section if present
    if "interpreter" in config:
        interpreter_config = config["interpreter"]
        if "max_tokens" in interpreter_config and not isinstance(interpreter_config["max_tokens"], int):
            logger.error("Interpreter max_tokens must be an integer")
            return False
        
        if "temperature" in interpreter_config and not isinstance(interpreter_config["temperature"], (int, float)):
            logger.error("Interpreter temperature must be a number")
            return False
        
        if "model" in interpreter_config and not isinstance(interpreter_config["model"], str):
            logger.error("Interpreter model must be a string")
            return False
        
        if "provider" in interpreter_config and not isinstance(interpreter_config["provider"], str):
            logger.error("Interpreter provider must be a string")
            return False
        
        if "api_key" in interpreter_config and interpreter_config["api_key"] is not None and not isinstance(interpreter_config["api_key"], str):
...
def create_default_questions_config():
    ...
    if not questions_config_path.exists():
        ...
        default_questions = {
            "1": ["Explain the differences between supervised and unsupervised learning with examples.", 10],
            "2": ["Describe how gradient descent works for optimizing a neural network.", 15],
            "3": ["Explain the bias-variance tradeoff and its significance in machine learning.", 10],
            "4": ["Define precision and recall, and explain their importance in classification tasks.", 10],
            "5": ["Explain how regularization helps prevent overfitting in machine learning models.", 10]
        }
        ...
        try:
            ...
            logger.info(f"Created default questions configuration at {questions_config_path}")
        except Exception as e:
