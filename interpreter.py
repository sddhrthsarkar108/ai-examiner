...
class QuestionContent(BaseModel):
    ...
    equations: List[str] = Field(
        default_factory=list, description="List of equations in the answer"
    )
    figures: List[str] = Field(
        default_factory=list, description="List of figures in the answer"
    )
    calculations: List[str] = Field(
        default_factory=list, description="List of calculations in the answer"
    )
    continues_from_previous: bool = Field(
        default=False, description="Whether this continues from a previous page"
    )
    continues_to_next: bool = Field(
        default=False, description="Whether this continues to the next page"
    )
...
class InterpretedAnswer(BaseModel):
    ...
    questions: Dict[str, QuestionContent] = Field(
        description="Dictionary of questions keyed by question number"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional metadata about the answer set"
    )
    summary: str = Field(default="", description="Summary of the answer content")


def create_interpreter_chain(
    ...
    def run_chain(ocr_data):
        ...
        try:
            ...
            try:
                ...
                all_text = "\n\n".join(
                    [f"{page}: {content}" for page, content in ocr_data.items()]
                )
            ...
            return {
                "questions": {
                    "error": {"question_number": "error", "raw_text": str(ocr_data)}
                },
...
def interpret_ocr_output(ocr_data, output_dir, app_config=None, api_key=None):
    ...
    config = (
        DefaultConfig.INTERPRETER.copy()
        if hasattr(DefaultConfig, "INTERPRETER")
        else {
            "temperature": 0.1,
            "max_tokens": 4000,
