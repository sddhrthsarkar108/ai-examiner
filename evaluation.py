...
def evaluate_answers(
    questions, answers, eval_output_dir, app_config=None, api_key=None, run_config=None
):
    ...
    questions_table = "\n".join(
        [
            f"Question {q_no}: {question_text} [{max_marks} marks]"
    ...
    question_score_template = "\n".join(
        [
            f'{q_no},{max_marks},your_score_for_q{q_no},"Brief justification for question {q_no} score"'
    ...
    with open(eval_output_dir / "api_request_log.txt", "w") as f:
        ...
        f.write(
            f"Sent evaluation request to {provider} {model_name} API with temperature={temperature}, max_tokens={max_tokens}\n"
        )
    ...
    try:
        ...
        # Run the chain
        evaluation_result = chain({"system": system_prompt, "user": user_prompt})
...
def parse_evaluation_results(evaluation_result, eval_output_dir, questions):
    ...
    table_match = re.search(
        r"<\s*SCORE_TABLE\s*>\s*(.*?)\s*<\s*/\s*SCORE_TABLE\s*>",
        evaluation_result,
    ...
    if not table_match:
        ...
        for q_no, (question_text, max_marks) in questions.items():
            logger.warning(
                f"Could not extract score for question {q_no}, assuming zero"
            )
        ...
        table_content = re.sub(
            r"^\s+|\s+$", "", table_content, flags=re.MULTILINE
        )  # Trim each line
        ...
        for line in lines[1:]:
            ...
            try:
                ...
                if (
                    len(parts) >= 1
                ):  # As long as we have a question number, we can process
                    ...
                    try:
                        ...
                        question_text, default_max_marks = questions.get(
                            q_no, ["Unknown question", 0]
                        )
                        ...
                        if len(parts) >= 2 and parts[1].strip():
                            try:
                                ...
                                logger.warning(
                                    f"Invalid max marks for question {q_no}, using default: {default_max_marks}"
                                )
                        ...
                        if len(parts) >= 3 and parts[2].strip():
                            try:
                                ...
                                logger.warning(
                                    f"Invalid score for question {q_no}, defaulting to 0"
                                )
                                score = 0.0
                        else:
                            logger.warning(
                                f"Missing score for question {q_no}, defaulting to 0"
                            )
                            score = 0.0
                        ...
                        if score > max_marks:
                            logger.warning(
                                f"Score for question {q_no} ({score}) exceeds max marks ({max_marks}), capping at max"
                            )
                            score = max_marks

                        summary.append(
                            [q_no, question_text, max_marks, score, justification]
                        )
                    except ValueError as e:
                        logger.warning(f"Error parsing score for question {q_no}: {e}")
                        # Use values from our questions dictionary as fallback
                        question_text, max_marks = questions.get(
                            q_no, ["Unknown question", 0]
                        )
                        summary.append(
                            [q_no, question_text, max_marks, 0, "Error parsing score"]
                        )
                else:
                    logger.warning(
                        f"Malformed line in score table (no question number): {line}"
                    )
            except Exception as e:
                logger.warning(
                    f"Error processing line in score table: {line}, Error: {e}"
                )
                ...
                if q_no_match:
                    ...
                    summary.append(
                        [q_no, question_text, max_marks, 0, "Error parsing score line"]
                    )
        ...
        for q_no in missing_questions:
            logger.warning(
                f"Question {q_no} missing from score table, adding with zero score"
            )
            question_text, max_marks = questions.get(q_no, ["Unknown question", 0])
            summary.append(
                [q_no, question_text, max_marks, 0, "Missing from score table"]
            )
    ...
    total_row = pd.DataFrame(
        [["Total", "", total_marks, obtained_marks, ""]],
        columns=[
            "Question No.",
            "Question",
            "Max Marks",
...
def generate_aggregate_summary(all_summaries, output_dir):
    ...
    """
    logger.info(
        "Generating aggregate summary across all evaluations using voting system..."
    )
    ...
    for _, row in template_df.iterrows():
        ...
        scores = [
            df.loc[df["Question No."] == q_no, "Marks Obtained"].values[0]
            for df in all_summaries
            if q_no in df["Question No."].values
        ]
        ...
        while len(scores) < num_runs:
            scores.append(0.0)
            logger.warning(
                f"Missing score for question {q_no} in at least one evaluation run"
            )
        ...
        most_common_scores = [
            score for score, count in score_counts.items() if count == max_vote_count
        ]
        ...
        justifications = [
            df.loc[df["Question No."] == q_no, "Justification"].values[0]
            for df in all_summaries
            if q_no in df["Question No."].values and "Justification" in df.columns
        ]
        ...
        justification_note = (
            " | ".join(justification_notes) if justification_notes else ""
        )
        ...
        aggregate_df = pd.concat(
            [aggregate_df, pd.DataFrame([row_data])], ignore_index=True
        )
    ...
    # Add total row
    aggregate_df = pd.concat(
        [aggregate_df, pd.DataFrame([total_data])], ignore_index=True
    )
    ...
    for i in range(num_runs):
        ...
        aggregate_df[percentage_column] = aggregate_df.apply(
            lambda row: (
                round(row[run_column] / row["Max Marks"] * 100, 1)
                if row["Max Marks"] > 0
                else 0
            ),
            axis=1,
    ...
    aggregate_df["Voting %"] = aggregate_df.apply(
        lambda row: (
            round(row["Voting Score"] / row["Max Marks"] * 100, 1)
            if row["Max Marks"] > 0
            else 0
        ),
        axis=1,
    ...
    aggregate_df["Average %"] = aggregate_df.apply(
        lambda row: (
            round(row["Average Score"] / row["Max Marks"] * 100, 1)
            if row["Max Marks"] > 0
            else 0
        ),
        axis=1,
            ...
            if s.name in [f"Run {i+1} Score" for i in range(num_runs)]:
                ...
                return [
                    (
                        ...
                        if v
                        else "background-color: #FFC7CE" if m else ""
                    )
