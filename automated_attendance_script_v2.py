...
def update_attendance(image_path):
    ...
    df["Roll Number"] = df["Roll Number"].astype(str)  # Ensure roll numbers are strings

    # Create a new DataFrame based on STUDENT_DICT to fix name inconsistencies
    corrected_df = pd.DataFrame({"Name": list(STUDENT_DICT.keys()), "Roll Number": list(STUDENT_DICT.values())})
    ...
    if today_date not in df.columns:
        df[today_date] = "Absent"  # Default everyone to "Absent"
