...
class LLMFactory:
    ...
    def _get_api_key(cls, provider: str, config_api_key: Optional[str] = None) -> str:
        ...
        # No key found
        raise ValueError(
            f"API key not provided for {provider}. Please set {env_var_name} environment variable or provide it in app_config.json"
        )
    ...
    def create_llm(
        ...
        **kwargs,
        ...
        if provider == "openai":
            ...
            raise ValueError(
                f"Unsupported LLM provider: {provider}. Supported providers are: openai, google, deepseek"
            )
        ...
        def run_chain(inputs):
            try:
                ...
                if provider == "deepseek":
                    ...
                    messages = [("system", inputs["system"]), ("human", inputs["user"])]
                else:
